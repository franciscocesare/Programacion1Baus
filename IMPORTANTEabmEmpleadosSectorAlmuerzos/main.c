...
int menu();
void mostrarAlmuerzo(eAlmuerzo almuerzo, eEmpleado empleados[], int tamEmpleados, eComida comidas[], int tamComidas);
void mostrarComidasXSector(eAlmuerzo almuerzos[], int tamAlmuerzo, eEmpleado empleados[], int tamEmpleados, eComida comidas[], int tamComidas, eSector sectores[], int tamSector);
void mostrarComidas(eComida comidas[], int tam);
void mostrarComida(eComida unaComida);
void mostrarComidasXSector(eAlmuerzo almuerzos[], int tamAlmuerzo, eEmpleado empleados[], int tamEmpleados, eComida comidas[], int tamComidas, eSector sectores[], int tamSector);

///DESARROLLO DEL MAIN Y LLAMADO DE FUNCIONES
...
int main()
{
    ...
    eComida comidas[] = ///HARDCODEO DE ESTRUC COMIDAS
    {
        {1, "Milanesa"}, ///ID, DESCRIPCION
        {2, "Fideos"},
        {3, "Pizza"},
        {4, "Sopa"},
        {5, "Pescado"}
    ...
    eAlmuerzo almuerzos[] = ///HARDCODEO DE ESTRUC ALMUERZOS
    {
        {100, 1111, 2},
        {101, 5555, 1}, ///ID, IdEMPLEADO, IdCOMIDA
        {102, 4545, 3},
        {103, 3232, 4},
        {104, 1111, 1},
        {105, 5555, 5},
    ...
    inicializarEmpleados(lista, 10); ///LE PASO EL VECTOR Y EL TAMANO
    ...
    do
    {
        ...
        case 5:
            ...
            break;
        case 6:
            //sortEmployeesByLastName(lista, 10);
        ...
        case 9:
            ...
            break;
        case 10:
            mostrarComidasXSector(almuerzos, 21, lista, 10, comidas, 5, sectores, 5);
            system("pause");
            break;
        case 11:
            seguir = 'n';
...
void mostrarAlmuerzos(eAlmuerzo almuerzos[], int tamAlmuerzo, eEmpleado empleados[], int tamEmpleados, eComida comidas[], int tamComidas)
{
    ...
    {
        mostrarAlmuerzo(almuerzos[i], empleados, tamEmpleados, comidas, tamComidas);
    }
...
void mostrarAlmuerzo(eAlmuerzo almuerzo, eEmpleado empleados[], int tamEmpleados, eComida comidas[], int tamComidas)
{
    ...
    {
        if(almuerzo.idEmpleado == empleados[j].legajo)
        {
    ...
    {
        if(almuerzo.IdComida == comidas[k].id)
        {
    ...
    printf("%d  %d %10s   %10s\n", almuerzo.id, almuerzo.idEmpleado, nombreEmpleado, descComida);
...
void mostrarComida(eComida unaComida){
    ...
    printf(" %d    %s\n", unaComida.id, unaComida.descripcion );
}
...
void mostrarComidasXSector(eAlmuerzo almuerzos[], int tamAlmuerzo, eEmpleado empleados[], int tamEmpleados, eComida comidas[], int tamComidas, eSector sectores[], int tamSector){
    ...
    for(int i=0; i < tamEmpleados; i++){
        ...
        if( empleados[i].isEmpty == 1 && empleados[i].idSector == idSector){

            for(int j=0; j < tamAlmuerzo; j++){

               if( almuerzos[j].idEmpleado == empleados[i].legajo){

                    mostrarAlmuerzo(almuerzos[j], empleados, tamEmpleados, comidas, tamComidas);
               }
            }
        }
    }
